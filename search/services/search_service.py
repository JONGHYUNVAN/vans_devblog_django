...
class SearchService:
    ...
    def search_posts(self, search_params: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            filters = self._build_filters(theme, category, tags, language, date_from, date_to)
            
            cached_result = self.cache_service.get_search_result(query, filters, page, page_size)
            if cached_result:
            ...
            try:
                ...
                # 검색 로그 기록
                SearchLog.record_log(
                    query=query,
                    results_count=response_data['total']
                )
                ...
                logger.debug(f"Search log recorded: query='{query}', results={response_data['total']}")
                
            except Exception as log_error:
                logger.warning(f"Failed to record search log: {str(log_error)}")
            
            self.cache_service.set_search_result(query, filters, page, page_size, response_data)
            
            logger.info(f"Search completed: query='{query}', total={response_data['total']}, page={page}")
    ...
    def get_autocomplete_suggestions(self, autocomplete_params: Dict[str, Any]) -> Dict[str, Any]:
        try:
            query = autocomplete_params.get('query', '')
            language = autocomplete_params.get('language', 'all')
            limit = autocomplete_params.get('limit', 10)
            
            cached_suggestions = self.cache_service.get_autocomplete_suggestions(query, language)
            ...
            self.cache_service.set_autocomplete_suggestions(query, language, suggestions)
    ...
    def get_popular_searches(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to get popular searches from DB: {str(db_error)}")
    ...
    def _build_filters(self, theme: str, category: str, tags: List[str], 
                      language: str, date_from: Any, date_to: Any) -> Dict[str, Any]:
        filters = {
            'theme': theme,
            'category': category,
            'tags': tags,
    ...
    def _build_search_response(self, search_result: Dict[str, Any], 
