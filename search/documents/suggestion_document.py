...
from elasticsearch_dsl import Document, Completion, Keyword, Integer
import logging
...
class SuggestionDocument(Document):
    ...
    def save(self, **kwargs) -> 'SuggestionDocument':
        ...
        try:
            ...
            logger.error(f"Failed to save suggestion document {self.suggestion}: {str(e)}")
            raise
    
    @classmethod
    def create_suggestion(cls, suggestion_text: str, suggestion_type: str, 
                         language: str = 'ko', frequency: int = 1) -> 'SuggestionDocument':
        """
        제안 문서를 생성합니다.
