...
class TestSearchAPI:
    ...
    def test_search_posts_endpoint(self, api_client, mock_elasticsearch, mock_mongodb):
        ...
        with patch('search.services.search_service.SearchService.search_posts', 
    ...
    def test_autocomplete_endpoint(self, api_client, mock_elasticsearch, mock_mongodb):
        """자동완성 엔드포인트 테스트"""
        mock_suggestions = {
            'suggestions': ['Django', 'Django REST', 'Django Testing']
        }
        
        with patch('search.services.search_service.SearchService.autocomplete', 
    ...
    def test_popular_searches_endpoint(self, api_client, mock_elasticsearch, mock_mongodb):
        """인기 검색어 엔드포인트 테스트"""
        mock_popular = {
            'popular_searches': [
        ...
        with patch('search.services.search_service.SearchService.get_popular_searches', 
    ...
    def test_categories_endpoint(self, api_client, mock_elasticsearch, mock_mongodb):
        ...
        with patch('search.services.search_service.SearchService.get_categories', 
...
class TestAPIErrorHandling:
    ...
    def test_search_service_exception(self, api_client, mock_elasticsearch, mock_mongodb):
        """검색 서비스 예외 처리 테스트"""
        with patch('search.services.search_service.SearchService.search_posts', 
    ...
    def test_autocomplete_service_exception(self, api_client, mock_elasticsearch, mock_mongodb):
        """자동완성 서비스 예외 처리 테스트"""
        with patch('search.services.search_service.SearchService.autocomplete', 
...
class TestAPIPerformance:
    ...
    def test_search_response_structure(self, api_client, mock_elasticsearch, mock_mongodb):
        """검색 응답 구조 테스트"""
        mock_result = {
            'total': 10,
            'page': 1,
