...
class TestSearchService:
    ...
    def test_search_posts_with_logging(
        self,
        mock_popular_update,
        mock_log_record,
        mock_elasticsearch,
        ...
        result = service.search_posts(
            {"query": "Django Test", "page": 1, "page_size": 20}
        )
        ...
        # 로그 기록 검증
        mock_log_record.assert_called_once_with(query="Django Test", results_count=2)
    ...
    def test_search_posts_empty_query(self, mock_elasticsearch, mock_mongodb):
        ...
        service = SearchService()
        result = service.search_posts({"query": "", "page": 1, "page_size": 20})

        assert result["total"] == 0
        assert result["results"] == []

    @patch("search.models.PopularSearch.get_top_popular_searches")
    def test_get_popular_searches_from_db(
        self, mock_get_popular, mock_elasticsearch, mock_mongodb, clean_cache
    ):
        """DB에서 인기 검색어 조회 테스트"""
    ...
    def test_get_popular_searches_empty_db(
        self, mock_elasticsearch, mock_mongodb, clean_cache
    ):
        """DB가 비어있을 때 인기 검색어 조회 테스트"""
        with patch(
            "search.models.PopularSearch.get_top_popular_searches", return_value=[]
        ):
            service = SearchService()
            result = service.get_popular_searches()
    ...
    def test_get_categories_from_mongodb(
        self, mock_elasticsearch, mock_mongodb, clean_cache
    ):
...
class TestHealthService:
    ...
    def test_health_check_elasticsearch_unhealthy(
        self, mock_elasticsearch, mock_mongodb
    ):
